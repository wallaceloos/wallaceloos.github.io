...
<p align="justify">At the heart of the attention mechanism are three concepts: Key, Query, and Value. These are essentially linear transformations of the input data. Initially, after word embedding, each vector only encodes the meaning of that particular word. The intuition behind Key, Query, and Value is to provide context to the words. You can think of the Query as a question that the word is asking, and the Key as the answer to that question. For instance, the Query could be “Is there any verb in front of me?”, and the words that are verbs can answer “I am a verb!”. The result of the Query and Key interaction is an attention map that shows the relevance of each word compared to the others. The Value updates the encoding vector by introducing more context to that specific word. This is performed through a weighted sum between the attention map and the elements from the Values. The result of this sum is added to the encoding vectors [1].
